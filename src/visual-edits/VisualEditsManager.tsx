...
export const CHANNEL = "PASTEL_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "pastel_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "pastel_focused_element" as const;
...
const postMessageDedup = (data: any) => {
  try {
    ...
    if (key === _pastelLastMsg) return; // identical – drop
    _pastelLastMsg = key;
  } catch {
...
// Helper to parse data-pastel-id to extract file path, line, and column
const parsePastelId = (
  pastelId: string,
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = pastelId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-pastel-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-pastel-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the pastel ID from the element to ensure we're working with the right one
    const pastelId = element.getAttribute("data-pastel-id");
    if (!pastelId) return;
    ...
    if (newText !== oldText) {
      const parsed = parsePastelId(pastelId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same pastel ID
    const allMatchingElements = document.querySelectorAll(
      `[data-pastel-id="${pastelId}"]`,
    ...
    const existingStyles = appliedStylesRef.current.get(pastelId) || {};
    appliedStylesRef.current.set(pastelId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(pastelId);
  ...
  const flushImageSrcChange = () => {
    ...
    const pastelId = imgElement.getAttribute("data-pastel-id");
    if (!pastelId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "PASTEL_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same pastel ID
        const allMatchingElements = document.querySelectorAll(
          `[data-pastel-id="${elementId}"]`,
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "PASTEL_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-pastel-id
        const pastelId = element.getAttribute("data-pastel-id");
        if (pastelId) {
          const parsed = parsePastelId(pastelId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-pastel-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-pastel-id");
        ...
        const tagName =
          hit.getAttribute("data-pastel-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same pastel ID
        const allMatchingElements = document.querySelectorAll(
          `[data-pastel-id="${hitId}"]`,
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-pastel-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-pastel-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same pastel ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-pastel-id="${hitId}"]`,
        ) as NodeListOf<HTMLElement>;
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same pastel ID
        const allMatchingElements = document.querySelectorAll(
          `[data-pastel-id="${e.data.elementId}"]`,
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-pastel-name") ||
    ...
    <>
      ...
      {isVisualEditMode && !isResizing && hoverBox && (
        <>
          ...
          <div
            className="fixed pointer-events-none border-[0.5px] border-[#0ea5e9] bg-sky-300/20 border-dashed"
            style={{
              zIndex: 100000,
              left: hoverBox.left,
              top: hoverBox.top,
          ...
          {hoverTag && (
            <div
              className="fixed pointer-events-none text-[10px] text-white bg-[#0ea5e9] px-1 py-0.5 rounded-[2px]"
              style={{
                zIndex: 100001,
                left: hoverBox.left,
                top: hoverBox.top - 20,
      ...
      {isVisualEditMode && focusBox && (
        <>
          {focusTag && (
            <div
              className="fixed text-[10px] font-semibold text-white bg-[#0ea5e9] px-1 rounded-[2px] pointer-events-none select-none"
              style={{
                zIndex: 100003,
          ...
          <div
            className="fixed pointer-events-none border-[1.5px] border-[#0ea5e9]"
            style={{
              zIndex: 100001,
              left: focusBox.left,
              top: focusBox.top,
          ...
          {!isResizing && (
            <>
              {/* Corner handles */}
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-nw-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
                  left: focusBox.left - 4,
                  top: focusBox.top - 4,
              ...
              />
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-ne-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
              ...
              />
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-sw-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
                  left: focusBox.left - 4,
              ...
              />
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-se-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
                  left: focusBox.left + focusBox.width - 4,
                  top: focusBox.top + focusBox.height - 4,
              ...
              {/* Edge handles */}
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-n-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
              ...
              />
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-s-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
                  left: focusBox.left + focusBox.width / 2 - 4,
                  top: focusBox.top + focusBox.height - 4,
              ...
              />
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-w-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
                  left: focusBox.left - 4,
              ...
              />
              <div
                className="fixed w-2 h-2 bg-[#0ea5e9] rounded-[2px] cursor-e-resize pointer-events-auto resize-handle"
                style={{
                  zIndex: 100002,
                  left: focusBox.left + focusBox.width - 4,
                  top: focusBox.top + focusBox.height / 2 - 4,
